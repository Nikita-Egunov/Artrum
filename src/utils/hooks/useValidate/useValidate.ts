...
export default function useValidate() {
  ...
  const removeErrorClasses = (input: HTMLInputElement, span: HTMLSpanElement) => {
  ...
  const addSpanError = (input: HTMLInputElement, span: HTMLSpanElement, errors: string[]) => {
    addErrorClasses(input, span)

    const errorString = errors.join(', ').toLowerCase();
    const capitalizedErrorString = errorString.charAt(0).toUpperCase() + errorString.slice(1);
    ...
    switch (input.type) {
      ...
      case 'password': {
        if (input.name === 'password') {
          ...
          const passwordInput = input.closest('form')?.querySelector('input[name="password"]') as HTMLInputElement
  ...
  const submit = (form: HTMLFormElement, setLoading: (arg0: boolean) => void, setRespType: (arg0: "success" | "error") => void) => {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>, setLoading: (arg0: boolean) => void, setRespType: (arg0: "success" | "error") => void) => {
    e.preventDefault();
    ...
    inputs.forEach((input) => {
      ...
      const span = input.closest('label')?.querySelector('span') as HTMLSpanElement
